...
const float I_ext_s = 0;
const float I_ext_p = 0;
const float I_ext_d = 0.1;

__global__
void pyramStep(int n, float h, pyramNeuron* vpn, pyramReceptor* vpr)
{
    ...
    float I_p_sum = I_L_p + I_A_p + I_CaBK_p + I_Ca_p;
    float V_p_eq = (I_L_p * vL + I_A_p * vK + I_CaBK_p * vK + I_Ca_p * vCa);

    float I_d_sum = I_L_d + I_A_d;
...
void initPyramNeuron(pyramNeuron* pn) {
    pn[0].V_s = -64.6240844727f;
    pn[0].V_p = -64.5471725464f;
    pn[0].V_d = -64.8385238647f;
    pn[0].m_s = 0.0121233119f;
    pn[0].h_s = 0.9866571426f;
    pn[0].n_s = 0.0848252103f;
    pn[0].w_s = 2.4368e-06f;
    pn[0].Ca_s = 2.4334e-06f;
    pn[0].c1_s = 1.343e-07f;
    pn[0].r_s = 0.0025670978f;
    pn[0].a_p = 0.0025868677f;
    pn[0].b_p = 0.6896412373f;
    pn[0].Ca_p = 0.0002222956f;
    pn[0].c1_p = 1.351e-07f;
    pn[0].r_p = 0.0025868667f;
    pn[0].a_d = 0.0025127728f;
    pn[0].b_d = 0.6971943974f;
}
